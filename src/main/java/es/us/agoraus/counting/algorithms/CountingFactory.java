...
public class CountingFactory {
	...
	private static List<Vote> decodeVotes(Collection<Object> pollVotes) {
		...
		for (String stringVote : stringVotes) {
			...
			Vote vote = new Vote(id, age, gender, community);
	 ...
	 *         with the next format: 'id=voteId;count=numberOfVotes'
	 */
	public static Map<String, Object> basicCount(Map<String, Object> poll) {
		...
		for (String id : groupedVotes.keySet()) {
			stringGroupedVotes.add("id=" + id + ";count=" + groupedVotes.get(id));
		}
	 ...
	 */
	public static Map<String, Object> ageMayorityCount(Map<String, Object> poll) {
		Map<String, Object> countedPoll = new HashMap<String, Object>();
		List<Vote> votes = new ArrayList<Vote>();
		Map<String, Integer> minorsGroupedVotes = new HashMap<String, Integer>();
		Map<String, Integer> adultsGroupedVotes = new HashMap<String, Integer>();
		...
		for (Vote vote : votes) {
			...
			if (Integer.parseInt(vote.getAge()) < 18) {
				if (minorsGroupedVotes.containsKey(stringVoteId)) {
					minorsGroupedVotes.put(stringVoteId, minorsGroupedVotes.get(stringVoteId) + 1);
				} else {
					minorsGroupedVotes.put(stringVoteId, 1);
				}
			} else {
				if (adultsGroupedVotes.containsKey(stringVoteId)) {
					adultsGroupedVotes.put(stringVoteId, adultsGroupedVotes.get(stringVoteId) + 1);
				} else {
					adultsGroupedVotes.put(stringVoteId, 1);
				}
			}
		}
		...
		for (int i = 0; i < votesId.size(); i++) {
			Integer numberMinorsGroupedVotes;
			Integer numberAdultsGroupedVotes;
			numberMinorsGroupedVotes = minorsGroupedVotes.get(votesId.get(i).toString());
			if (numberMinorsGroupedVotes == null) {
				numberMinorsGroupedVotes = 0;
			}
			numberAdultsGroupedVotes = adultsGroupedVotes.get(votesId.get(i).toString());
			if (numberAdultsGroupedVotes == null) {
				numberAdultsGroupedVotes = 0;
			}
			stringGroupedVotes.add("id=" + votesId.get(i) + ";countMinors=" + numberMinorsGroupedVotes + ";countAdults="
					+ numberAdultsGroupedVotes);
		}
	...
	public static Map<String, Object> dhontCount(Map<String, Object> poll) {
		...
		List<Vote> votes = decodeVotes(poll.values());
		Map<String, Integer> groupedVotes = new HashMap<String, Integer>();
		votes = decodeVotes(poll.values());
	...
	public static Map<String, Object> usualAgeRanges(Map<String, Object> poll) {
		Map<String, Object> res = new HashMap<String, Object>();
		List<Vote> votes = new ArrayList<Vote>();
		Map<String, Integer[]> mapa = new HashMap<String, Integer[]>();
		votes = decodeVotes(poll.values());
		for (Vote v : votes) {
			Integer age = Integer.parseInt(v.getAge());
			if (age < 30) {
				if (mapa.containsKey(v.getId())) {
					Integer[] array = new Integer[5];

					array = mapa.get(v.getId());
					array[0] = array[0] + 1;
					mapa.put(v.getId(), array);
				} else {
					Integer[] array = new Integer[5];
					array[0] = 1;
					array[1] = 0;
		...
		for (String id : mapa.keySet()) {
			string.add("id=" + id + ";countUnder30=" + mapa.get(id)[0] + ";count30to45=" + mapa.get(id)[1]
					+ ";count45to60=" + mapa.get(id)[2] + ";count60to75=" + mapa.get(id)[3] + ";countOver75="
					+ mapa.get(id)[4]);
		}
		res.put(poll.keySet().iterator().next(), string);
		return res;
	}
